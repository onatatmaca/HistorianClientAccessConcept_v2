...
namespace HistorianSyncTool.Tests
{
    ...
    public class GapAnalysisServiceTests
    {
        ...
        // ── Minimum gap duration floor ─────────────────────────────────────────────

        [TestMethod]
        public void Analyze_DeadbandNoise_BelowFloor_IsNotFlagged()
        {
            // Simulates deadband-logged tag: mostly 10s intervals, occasional 30s quiet stretches.
            // Median ≈ 10s → old threshold would be 15s → every 30s quiet would be a "gap".
            // With 120s floor: no false gaps.
            var svc = new GapAnalysisService(TimeSpan.FromMinutes(10), TimeSpan.FromSeconds(120));
            var start = new DateTime(2025, 1, 1, 12, 0, 0);
            var times = new List<DateTime>();
            var t = start;
            for (int i = 0; i < 20; i++) { times.Add(t); t = t.AddSeconds(10); }
            t = t.AddSeconds(30); // a 30s "quiet" — real gap? No, just deadband silence
            for (int i = 0; i < 20; i++) { times.Add(t); t = t.AddSeconds(10); }

            var result = svc.Analyze(times, "Test");

            Assert.IsFalse(result.HasGaps,
                "30-second quiet stretch on a 10s-median tag should not be flagged when floor is 120s.");
        }

        [TestMethod]
        public void Analyze_RealGapAboveFloor_StillDetected()
        {
            // Floor should NOT hide legitimate gaps that are longer than the floor.
            var svc = new GapAnalysisService(TimeSpan.FromMinutes(10), TimeSpan.FromSeconds(120));
            var start = new DateTime(2025, 1, 1, 12, 0, 0);
            var times = new List<DateTime>();
            for (int i = 0; i < 10; i++) times.Add(start.AddSeconds(i * 10));
            // 5-minute gap (300s > 120s floor)
            for (int i = 0; i < 10; i++) times.Add(start.AddMinutes(5).AddSeconds(i * 10 + 100));

            var result = svc.Analyze(times, "Test");

            Assert.IsTrue(result.HasGaps, "Gap longer than floor should be detected.");
            Assert.AreEqual(1, result.Gaps.Count);
        }

        [TestMethod]
        public void Constructor_CustomMinGapDuration_IsUsed()
        {
            var svc = new GapAnalysisService(TimeSpan.FromMinutes(10), TimeSpan.FromSeconds(30));
            Assert.AreEqual(TimeSpan.FromSeconds(30), svc.MinGapDuration);

            // With floor=30s, a 60s gap on a 10s median tag SHOULD be flagged
            // (max(10s*1.5=15s, 30s) = 30s threshold; 60s > 30s = gap).
            var start = new DateTime(2025, 1, 1, 12, 0, 0);
            var times = new List<DateTime>();
            for (int i = 0; i < 10; i++) times.Add(start.AddSeconds(i * 10));
            times.Add(start.AddSeconds(9 * 10 + 60)); // 60s jump
            for (int i = 0; i < 10; i++) times.Add(start.AddSeconds(9 * 10 + 60 + (i + 1) * 10));

            var result = svc.Analyze(times, "Test");

            Assert.IsTrue(result.HasGaps, "60s gap should be detected with a 30s floor.");
        }
